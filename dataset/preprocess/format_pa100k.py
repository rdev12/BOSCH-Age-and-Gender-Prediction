...
def generate_data_description(save_dir, reorder):
    ...
    train_image_name = [pa100k_data['train_images_name'][i][0][0] for i in range(8000)]
    val_image_name = [pa100k_data['val_images_name'][i][0][0] for i in range(1000)]
    test_image_name = [pa100k_data['test_images_name'][i][0][0] for i in range(1000)]
    dataset.image_name = train_image_name + val_image_name + test_image_name

    dataset.label = np.concatenate((pa100k_data['train_label'][0:8000], pa100k_data['val_label'][0:1000], pa100k_data['test_label'][0:1000]), axis=0)
    print(dataset.label.shape)
    assert dataset.label.shape == (10000, 26)
    ...
    dataset.partition.train = np.arange(0, 8000)  # np.array(range(80000))
    dataset.partition.val = np.arange(8000, 9000)  # np.array(range(80000, 90000))
    dataset.partition.test = np.arange(9000, 10000)  # np.array(range(90000, 100000))
    dataset.partition.trainval = np.arange(0, 9000)  # np.array(range(90000))
